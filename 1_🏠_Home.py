...
# Stack for search history of each school
class School:
...
def display_search_results(school):
    with st.container():
        ...
        st.subheader("Search History for " + school.name)
        if school.search_history:
            for query in school.search_history:
                st.write(query)
        else:
            st.write("No search history for this school")
...
if search_query:
    if search_query in schools:
        ...
        school.add_to_search_history(search_query)
    else:
